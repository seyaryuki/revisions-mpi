...
liste_couples cons_couple(int64_t c, int v, liste_couples l){
...
int hash(int capacite, int64_t k){
    //Renvoie le haché d'un entier.
    double tmp_val = floor(capacite * k * phi);
    return ((int64_t) tmp_val) % capacite;
}

bool appartient_liste_cle(liste_couples l, int64_t c){
...
void ajoute_entree(dico* d, int64_t c, int v){
...
void supprime_entree(dico* d, int64_t c){
